...
export async function toggleFavorite({ contactId, favorite, path }) {
    try {
        await connectDB()
        ...
        const result = await Contact.updateOne({ _id: contactId }, updateData)
...
export async function getFavoriteContactsByUser({ userId }) {
    try {
        await connectDB()
